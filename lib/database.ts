import "server-only";
import { db, companies, questions, topics, companyQuestions, questionTopics, videoSolutions, VideoSolution } from "./db"; // <-- Import videoSolutions AND VideoSolution type
...
  console.log(`--- 2. Is DATABASE_URL loaded? ${process.env.DATABASE_URL ? "Yes" : "NO, IT IS UNDEFINED"}`);
  
  try {
...
  const filters: QuestionFilters = { // <-- TYPO 's' REMOVED FROM HERE
