...
import {
  ...
  videoSolutions,
  VideoSolution,
} from "./db"; // <-- Import videoSolutions AND VideoSolution type
...
export async function getQuestionsFromDatabase(
  ...
  try {
    ...
    const dedupedMap = new Map<number, QuestionWithDetails>();
    for (const q of transformedQuestions) {
      const key = q.id;
      const existing = dedupedMap.get(key);
      if (!existing) {
        q.allCompanies = [q.company];
        dedupedMap.set(key, q);
      } else {
        if (!existing.allCompanies!.includes(q.company)) {
          existing.allCompanies!.push(q.company);
        }
        ...
        if (
          ...
          (newRank === existingRank && q.frequency > existing.frequency)
        ) {
          existing.company = q.company;
          existing.timeframe = q.timeframe;
    ...
    return { questions: [], companies: [], totalCount: 0 }; // <-- TYPO 's' REMOVED FROM HERE
  }
}

export async function getCompanies(): Promise<Company[]> {
  console.log("---");
  console.log("--- 1. ATTEMPTING to call getCompanies() ---");
  console.log(
    `--- 2. Is DATABASE_URL loaded? ${process.env.DATABASE_URL ? "Yes" : "NO, IT IS UNDEFINED"}`
  );
  try {
    console.log("--- 3. Inside try block, calling db.select() ---");
    ...
    console.log(`--- 4. SUCCESS: Found ${result.length} companies ---`);
    return result; // <-- TYPO 's' REMOVED FROM HERE
  } catch (error) {
    console.log("--- 4. FAILED: Caught an error ---");
    console.error("!!!!!!!!!! THE REAL ERROR IS HERE !!!!!!!!!!");
    console.error(error);
    console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    return [];
  }
...
// --- NEW FUNCTION TO GET VIDEOS ---
export async function getVideoSolutions(questionId: number): Promise<VideoSolution[]> {
  console.log(`--- Fetching videos for questionId: ${questionId} ---`);
  try {
    const results = await db
      .select()
      .from(videoSolutions)
      .where(eq(videoSolutions.questionId, questionId));

    console.log(`--- Found ${results.length} videos ---`);
    return results;
  } catch (error) {
    console.error(`Error fetching videos for questionId ${questionId}:`, error);
    return [];
  }
}
// --- END OF NEW FUNCTION ---
