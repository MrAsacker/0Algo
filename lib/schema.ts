...
// --- NEW TABLE FOR VIDEO SOLUTIONS ---
export const videoSolutions = pgTable("video_solutions", {
  id: serial("id").primaryKey(),
  // This links our videos to the main 'questions' table
  questionId: integer('question_id')
    .notNull()
    .references(() => questions.id, { onDelete: 'cascade' }), // If a question is deleted, its videos are deleted too
  videoUrl: text('video_url').notNull(),
  // You could add title/source fields here later if you want
});
// --- END OF NEW TABLE ---
...
  questionTopics: many(questionTopics),
  // --- ADD THIS RELATION ---
  videoSolutions: many(videoSolutions), // A question can have many video solutions
}));
...
// -- VIDEO SOLUTIONS RELATIONS ---
export const videoSolutionsRelations = relations(videoSolutions, ({ one }) => ({
  question: one(questions, {
    fields: [videoSolutions.questionId],
    references: [questions.id],
  }),
}));
...
export const userChats = pgTable("user_chats", {
  // We use the Clerk User ID as the primary key.
  // This ensures one global chat history per user.
  userId: text("user_id").primaryKey(), 
  
  // Stores the array of messages: [{ role: 'user', content: '...' }, ...]
...
export type VideoSolution = typeof videoSolutions.$inferSelect;
export type NewVideoSolution = typeof videoSolutions.$inferInsert;
